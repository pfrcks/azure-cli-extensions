Describe 'AzureML Kubernetes Testing' {
    ...
    It 'Creates the extension and checks that it onboards correctly' {
        $output = az $Env:K8sExtensionName extension create -c $ENVCONFIG.arcClusterName -g $ENVCONFIG.resourceGroup --cluster-type connectedClusters --extension-type $extensionType --name $extensionName --release-train preview --config enableTraining=true allowInsecureConnections=true
        $? | Should -BeTrue

        $output = az $Env:K8sExtensionName extension show -c $ENVCONFIG.arcClusterName -g $ENVCONFIG.resourceGroup --cluster-type connectedClusters --name $extensionName
        $? | Should -BeTrue
    ...
    It "Performs a show on the extension" {
        $output = az $Env:K8sExtensionName extension show --cluster-name $ENVCONFIG.arcClusterName -g $ENVCONFIG.resourceGroup --cluster-type connectedClusters --name $extensionName
        $? | Should -BeTrue
    ...
    It "Runs an update on the extension on the cluster" {
        ...
        az $Env:K8sExtensionName extension update --cluster-name $ENVCONFIG.arcClusterName -g $ENVCONFIG.resourceGroup --cluster-type connectedClusters --name $extensionName --auto-upgrade-minor-version false
        $? | Should -BeTrue
    ...
    It "Lists the extensions on the cluster" {
        $output = az $Env:K8sExtensionName extension list --cluster-name $ENVCONFIG.arcClusterName -g $ENVCONFIG.resourceGroup --cluster-type connectedClusters
        $? | Should -BeTrue
    ...
    It "Deletes the extension from the cluster" {
        az $Env:K8sExtensionName extension delete --cluster-name $ENVCONFIG.arcClusterName -g $ENVCONFIG.resourceGroup --cluster-type connectedClusters --name $extensionName
        $? | Should -BeTrue

        # Extension should not be found on the cluster
        az $Env:K8sExtensionName extension show --cluster-name $ENVCONFIG.arcClusterName -g $ENVCONFIG.resourceGroup --cluster-type connectedClusters --name $extensionName
        $? | Should -BeFalse
    }
