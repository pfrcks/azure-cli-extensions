...
def create_k8s_extension(cmd, client, resource_group_name, cluster_name, name, cluster_type,
    ...
    if create_identity:
        identity,location = __create_identity(cmd, resource_group_name, cluster_name, cluster_type, cluster_rp)
        if identity is not None and location is not None:
            extension_instance.identity, extension_instance.location = identity,location

    # Try to create the resource
    return sdk_no_wait(
        no_wait, client.create, resource_group_name, cluster_rp, cluster_type, cluster_name, name, extension_instance)
...
def delete_k8s_extension(client, resource_group_name, cluster_name, name, cluster_type, no_wait=False):
    ...
    return sdk_no_wait(no_wait, client.delete, resource_group_name, cluster_rp, cluster_type, cluster_name, name)
...
def __create_identity(cmd, resource_group_name, cluster_name, cluster_type, cluster_rp):
    ...
    if cluster_rp == 'Microsoft.Kubernetes':
        ...
        # there are created by RP always
        return None,None
    else:
